...
local use_avg = (if debug then true else std.parseJson(std.extVar('use_avg')));
local use_fwd = (if debug then true else std.parseJson(std.extVar('use_fwd')));
...

{
  ...
  "train_data_path":  base_dir + "ADReSS_train_data.csv",
  ...
  "model": {
    ...
    "seq2vec_encoder": if use_avg then avg_pooling else bert_pooler,
    [if use_avg && use_fwd then "feedforward"]:{
